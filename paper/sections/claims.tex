...
The final two rows are deliberately separate. The robustness grid strengthens
the static-beacon result for information-safety efficiency, but it does not
make a universal operational prescription. Completed evacuations remain noisy
and regime dependent, which is consistent with the paper's central claim:
information control must be evaluated by coupled belief and safety effects
rather than by a single aggregate endpoint.
