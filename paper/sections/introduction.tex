...
The paper's core insight is that emergency communication should be evaluated
as a safety-control action, not as a monotonic information-delivery mechanism.
This view changes both the simulator design and the evaluation standard:
messages are judged by coupled belief and safety effects, and a policy can be
informative even if it does not maximize raw evacuation count. It also gives a
natural standard for generated-language guidance: an LLM message is only
paper-facing evidence if it remains bounded, replayable, validated, and
measured under the same ISE and HCI diagnostics as deterministic messages.
