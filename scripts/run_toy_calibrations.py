...
def run(paths: list[str]) -> dict:
    ...
    for path in paths:
        ...
        rows.append({
            ...
            "agents_total": len([agent for agent in sim.agents if not getattr(agent, "is_responder", False)]),
            "agents_evacuated": len(sim.completed_agents),
            "max_travel_time_s": max(sim.travel_times_s) if sim.travel_times_s else None,
            "mean_travel_time_s": sum(sim.travel_times_s) / len(sim.travel_times_s) if sim.travel_times_s else None,
            "connector_usage": dict(getattr(sim, "connector_usage_cumulative", {})),
            "impossible_floor_jumps": list(getattr(sim, "impossible_floor_jumps", [])),
        })
    return {"ok": all(row["validation_ok"] and row["assertions_ok"] for row in rows), "scenarios": rows}


def main() -> None:
    parser = argparse.ArgumentParser(description="Run toy calibration/regression scenarios.")
