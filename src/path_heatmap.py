...
def setup_simulation(layout):
    ...
    im = ax2.imshow(path_density, cmap='Greens', interpolation='nearest', origin='lower')
    plt.colorbar(im, ax=ax2)
    ax2.set_title("Path Density Heatmap")
    return fig, ax1, ax2, np.array(people), exit_point, walls, path_density, im

def update(frame, scatter, people_container, exit_point, walls, ax1, ax2, path_density, im):
    """
...
def run_simulation(layout_file):
    ...
    fig, ax1, ax2, people, exit_point, walls, path_density, im = setup_simulation(layout)
    scatter = ax1.scatter(people[:, 0], people[:, 1], c='blue', s=10)
    people_container = [people]
    anim = FuncAnimation(fig, update, fargs=(scatter, people_container, exit_point, walls, ax1, ax2, path_density, im),
