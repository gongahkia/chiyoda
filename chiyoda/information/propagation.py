    ...
    def exchange(
        ...
        for exit_pos, sender_eb in sender_beliefs.exit_beliefs.items():
            ...
            should_transfer = (
                receiver_eb is None
                or (
                    sender_eb.freshness < receiver_eb.freshness
                    and sender_eb.source_credibility
                    > receiver_eb.source_credibility * 0.8
                )
            ...
            if should_transfer:
                ...
                distorted_congestion = max(
                    ...
                    min(
                        1.0,
                        sender_eb.congestion_est + np.random.normal(0, distortion * 2),
                    ),
                )

                new_credibility = sender_credibility * (
                    1.0 - distortion * sender_eb.hop_count
                )
        ...
        for sender_hb in sender_beliefs.hazard_beliefs:
            ...
            if matching_belief is not None:
                if transferred_credibility > matching_belief.source_credibility:
                    ...
                    matching_belief.freshness = min(
                        matching_belief.freshness, sender_hb.freshness + 0.05
                    )
                    matching_belief.source_credibility = transferred_credibility
                    matching_belief.hop_count = min(
                        matching_belief.hop_count, sender_hb.hop_count + 1
                    )
                    transferred = True
                continue
        ...
        if (
            sender_beliefs.general_danger_level
            > receiver_beliefs.general_danger_level + 0.1
        ):
            blended = (
