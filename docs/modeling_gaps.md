...
|:--|:--|:--|
| Real-station prediction | No station case is validated against drill, incident, or expert-coded station operations. | `validation-evidence-audit` makes external evidence explicit; station scenarios now record file-backed trajectory/reference artifacts, but benchmark claim tiers still downgrade when operational evidence is absent. |
| Pedestrian dynamics | Social-force and bottleneck behavior are grid-scale approximations. The Wuppertal proxy still differs from the lab reference flow. | `calibration-audit` exposes SFM profile/provenance and cohort bounds; Juelich/Wuppertal checks remain diagnostic, not certification. Local smoke visibility and flood depth slow movement explicitly; door-specific-flow speed limits are opt-in. |
| Hazard physics | Built-in gas, smoke, fire, flood, quake, shooter, wildfire, and ember fields are stylized. | `hazard-audit` labels imported vs stylized hazards; benchmark claim tiers downgrade stylized hazards. Fire/flood/terrain damage can now close topology, and hazard dose is bucketed by toxic, smoke, heat, flood, trauma, and crush load. Hydrology, combustion, toxicology, and structural collapse remain simplified. |
...
| Scenario validation | Static validation checks topology, starts, exits, and reachability. | Runtime assertions now cover behavioral, hazard, vertical-transport, and hostile/LLM metrics. Falsification tests verify bad topology, blocked evacuation, and unsafe guidance are rejected; these remain regression checks, not external validation. |
